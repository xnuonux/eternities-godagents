...
const certificationId = 'provider-resolution-authority-handoff-v1';
const protocolId = 'eternities-provider-resolution-authority-handoff-certification-v1';
const fixturePath = 'fixtures/provider-resolution-authority-handoff-v1.json';
const receiptPath = 'receipts/provider-resolution-authority-handoff-v1.json';
const specificationPath = 'docs/superpowers/specs/2026-08-31-provider-resolution-authority-handoff-v1-design.md';
const planPath = 'docs/superpowers/plans/2026-08-31-provider-resolution-authority-handoff-v1.md';
const certificationPath = 'docs/provider-resolution-authority-handoff-v1-certification.md';
...
const historicalReceiptPaths = Object.freeze([
  ...
  'provider-phase-host-sdk-v1.json', 'provider-resolution-decision-preparer-v1.json',
  'provider-resolution-profile-v1.json', 'receipt-bound-typed-executor-bundle-v1.json',
...
const implementationFiles = Object.freeze([
  ...
  specificationPath, 'scripts/build-provider-resolution-authority-handoff-v1-fixture.mjs',
...
const testFiles = Object.freeze([
  ...
  'tests/helpers/provider-resolution-authority-handoff-certification-fixture.mjs',
  'tests/provider-resolution-authority-handoff-certification.test.mjs',
  'tests/provider-resolution-authority-handoff.test.mjs',
  'tests/release-lineage.test.mjs',
].sort());
const focusedTestFiles = Object.freeze(['tests/provider-resolution-authority-handoff.test.mjs']);
const releaseOnlyPaths = Object.freeze([certificationPath, receiptPath]);
const requirements = Object.freeze([
  ['PRA-001', 'both provider families emit exact canonical signing requests'],
  ['PRA-002', 'requests bind host policies authority operation decision witness and signing bytes'],
  ['PRA-003', 'raw provider responses credentials paths and mutable handles remain absent'],
  ['PRA-004', 'signed returns bind one exact request decision witness and canonical signature encoding'],
  ['PRA-005', 'returns remain explicitly unauthenticated until the real controller accepts them'],
  ['PRA-006', 'both real controllers accept valid external signatures without redispatch'],
  ['PRA-007', 'cross-family changed-binding and authority-shaped input fail closed'],
  ['PRA-008', 'fixture source manifests trust roots ledger and lineage reproduce exactly'],
].map(([id, evidence]) => ({ id, status: 'pass', evidence: [evidence] })));
const proofLimits = Object.freeze([
  'structural handoff verification does not authenticate an Ed25519 signature',
  'the external authority and existing provider controller remain the cryptographic trust boundary',
  'no durable outbox external review provider truth or remote exactly-once execution is certified',
...
function verifyFixture(value) {
  exactKeys(value, ['schemaVersion', 'protocolId', 'families', 'assertions', 'fixtureDigest'], 'handoff fixture');
  if (value.schemaVersion !== 1 || value.protocolId !== 'eternities-provider-resolution-authority-handoff-fixture-v1'
      || !same(Object.keys(value.families), ['anthropic-messages-v1', 'openai-compatible-chat-completions-v1'])) {
    throw new Error('handoff fixture identity is invalid');
  }
  for (const family of Object.values(value.families)) {
    exactKeys(family, [
      'hostDescriptionDigest', 'transportPolicyDigest', 'resolutionPolicyDigest',
      'requestDigest', 'decisionDigest', 'responseWitnessDigest', 'signingPayloadSha256',
      'envelopeDigest', 'signatureAlgorithm', 'signingPayloadEncoding', 'cryptographicStatus',
    ], 'handoff family');
    for (const field of [
      'hostDescriptionDigest', 'transportPolicyDigest', 'resolutionPolicyDigest',
      'requestDigest', 'decisionDigest', 'responseWitnessDigest', 'signingPayloadSha256', 'envelopeDigest',
    ]) digest(family[field], field);
    if (family.signatureAlgorithm !== 'Ed25519' || family.signingPayloadEncoding !== 'utf-8'
        || family.cryptographicStatus !== 'unverified') throw new Error('handoff family authority semantics are invalid');
  }
  const expected = {
    families: 2, signingRequests: 2, signedReturns: 2, providerCalls: 0,
    embeddedPrivateKeys: 0, embeddedSigningOperations: 0, rawResponseBodiesRetained: 0,
    explicitUnverifiedReturns: 2, distinctRequests: 2, distinctReturns: 2,
  };
  if (!same(value.assertions, expected)) throw new Error('handoff fixture metrics are invalid');
  const { fixtureDigest, ...unsigned } = value;
  digest(fixtureDigest, 'fixture digest');
  if (fixtureDigest !== sha256Value(unsigned)) throw new Error('handoff fixture digest mismatch');
  return value;
}

export function verifyProviderResolutionAuthorityHandoffReceipt(value) {
  exactKeys(value, [
  ...
  if (value.schemaVersion !== 1 || value.certificationId !== certificationId
      || value.status !== 'certified' || value.protocolId !== protocolId) throw new Error('handoff receipt identity is invalid');
  ...
  if (!COMMIT.test(value.source.commit)
      || !same(Object.keys(value.source.historicalReceiptDigests), historicalReceiptPaths)) throw new Error('handoff history is invalid');
  ...
  if (value.fixture.path !== fixturePath || value.fixture.logicalDigest !== fixture.fixtureDigest
      || value.fixture.fileSha256 !== sha256Text(`${canonicalJson(fixture)}\n`)) throw new Error('handoff fixture binding is invalid');
  if (!same(value.requirements, requirements) || !same(value.metrics, fixture.assertions)) throw new Error('handoff metric evidence is invalid');
  verifyTestRuns(value.testRuns);
  if (!same(value.review, { mode: 'inline-adversarial', independent: false,
    unresolvedCriticalDefects: 0, unresolvedImportantDefects: 0 })) throw new Error('handoff review is invalid');
  if (!same(value.proofLimits, proofLimits)) throw new Error('handoff proof limits are invalid');
  const { receiptDigest, ...unsigned } = value;
  digest(receiptDigest, 'receipt digest');
  if (receiptDigest !== sha256Value(unsigned)) throw new Error('handoff receipt digest mismatch');
  return value;
}

export async function buildProviderResolutionAuthorityHandoffReceiptFromSource({ repositoryRoot, sourceCommit, testRuns } = {}) {
  ...
  return Object.freeze(verifyProviderResolutionAuthorityHandoffReceipt({ ...unsigned, receiptDigest: sha256Value(unsigned) }));
}
...
async function main() {
  ...
  const preliminary = await buildProviderResolutionAuthorityHandoffReceiptFromSource({
  ...
  const receipt = await buildProviderResolutionAuthorityHandoffReceiptFromSource({
  ...
  await writeFile(join(root, ...certificationPath.split('/')), `# Provider Resolution Authority Handoff v1 Certification\n\n- status: certified\n- source commit: \`${sourceCommit}\`\n- receipt digest: \`${receipt.receiptDigest}\`\n- fixture digest: \`${receipt.fixture.logicalDigest}\`\n- focused tests: ${focused.tests}\n- full tests: ${full.tests}\n- release tests: ${release.tests}\n\nThis certifies deterministic authority-neutral resolution signing exchange across both provider families. Cryptographic authentication remains exclusively with the existing provider controller.\n`, 'utf8');
  process.stdout.write(`${canonicalJson({ status: 'certified', sourceCommit, receiptDigest: receipt.receiptDigest,
