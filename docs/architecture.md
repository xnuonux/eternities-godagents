...
The [Godagent profile contract](superpowers/specs/2026-09-05-agent-profile-contract-v1-design.md) is the shared semantic boundary behind creation validation and body-free Godskills eligibility. It normalizes one closed profile policy, accepts an all-rounder or specialist, and binds a maximum composition of one to three. All-rounders are complete over the supplied metadata catalog by default; legacy preferred-family values on an all-rounder are inert. Explicit prohibited families and capability ids remain authoritative for either profile. Specialist preferences are ranking metadata only, so non-prohibited non-preferred rows remain eligible, and contradictory specialist preference/prohibition families fail closed. Empty catalogs retain the legacy-compatible empty result. The evaluator uses locale-independent canonical ordering and emits a digest-bound schema-validated result without loading skill bodies or changing the existing Godskills output shape. The [profile contract receipt](../receipts/agent-profile-contract-v1.json) proves this deterministic boundary only, not routing quality, model quality, authority, Realm, identity, continuity, keel, memory, evolution, Soul, Inspiration, Lunari, or default host adoption.
