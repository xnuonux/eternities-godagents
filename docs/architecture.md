...
The certification ledger verifier checks canonical receipt bytes, each receipt's internal digest, certified status, source-commit form, the exact registered receipt set, and declared historical file hashes across versioned receipt layouts. It does not rerun tests, builds, failure injection, or external review. Proof reproduction still requires checking out each receipt's pinned source commit and running its certifier.
