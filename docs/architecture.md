...
Verified module bytes are imported directly from data URLs keyed by bundle, capability, and source digest, so the executed source is the source already hashed rather than a later filesystem read and distinct executor identities do not share a module cache entry. The admitted host then independently repeats descriptor-policy matching and retains its policy, admission, and descriptor-bound durable namespace. Fresh-process crash recovery proves no dependence on the interrupted process's ESM state. This closes ordinary caller substitution and verify-then-import file mutation on the new path. Receipt-certified behavior remains trusted. The boundary does not sandbox code inside the Node process, prove hostile process-memory or same-user filesystem replacement isolation, distinguish hard-link identity, remove the executor-return publication window, establish external exactly-once effects, qualify live providers, or change any default.
