...
The certification ledger verifier checks canonical receipt bytes, each receipt's internal digest, certified status, source-commit existence in the current Git object database, the exact registered receipt set, and each version's exact required historical file set and hashes. It does not rerun tests, builds, failure injection, external review, or prove that a resolving commit is an ancestor of the current branch. Proof reproduction still requires checking out each receipt's pinned source commit and running its certifier.
