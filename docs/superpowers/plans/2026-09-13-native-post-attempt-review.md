...
- Preserve native input/output/cache/unknown usage. No fabricated split or zero-filled review-v1 conversion. Reserve the configured per-response maximum before every native or compaction inference, including retries; explicitly pass the cap and veto observed overrun/unknown successful output. This is reservation plus observed-usage enforcement, not a billing guarantee. Raw SDK reasoning counters remain separate native-transcript evidence and are not added to inclusive output.
...
- [x] Run one preregistered Grok4.6 subscription review on a separate copied snapshot. Native reads/settlement and fresh-process offline replay passed, but the reviewer missed the known invalid-date defects. Preserve that false negative and original output. A reported output overrun exposed a host check gap; four red regressions preceded its correction, followed by 88 focused passes and one actual compaction-overrun pass. A separate short transport qualification captured the outgoing cap128, returned READY with80 output tokens, and passed fresh offline replay. It was not a second quality review.
- [x] Run full release verification, preserve historical receipts, reconcile upstream, merge, verify the merged boundary and push. The initial full gate passed1641; the live-discovered usage correction required a new gate, which passed1646, zero failures and nine existing optional skips. Merge/push `f666d78` passed89 merged native checks. Review execution, observed-budget enforcement and reviewer accuracy remain separate claims; the inaccurate review was not rewritten into success.
