...
import { createAdmittedPortableIdentityLauncher } from '../../src/host/admitted-portable-identity-launcher.mjs';
...
const FIXTURE_PROTOCOL = 'eternities-admitted-portable-identity-launcher-fixture-v1';
const SECRET = 'admitted-portable-identity-launcher-certification-secret';

function requireCondition(condition, message) {
  if (!condition) throw new Error(`admitted portable launcher fixture failed: ${message}`);
}
...
function phaseOutput(input) {
  if (input.phase === 'native') {
    return { content: 'admitted portable draft with one evidence gap' };
  }
  if (input.phase === 'revision') {
    return {
      addressedFindingIds: ['bind-evidence'],
      content: 'admitted portable revision with the evidence gap repaired exactly',
    };
  }
  ...
  return {
    ...
    findings: [],
    summary: 'the exact revision is accepted',
  };
}
...
const FAMILIES = Object.freeze({
  ...
  'anthropic-messages-v1': Object.freeze({
    ...
    input(body) {
      return JSON.parse(body.messages[0].content[0].text);
    },
    credentialMatches(headers) {
    ...
    response(model, phase, ordinal, content) {
      return {
        id: `msg-admitted-portable-launcher-${phase}-${ordinal}`,
        type: 'message',
        role: 'assistant',
        model,
...
function requestFixture(family) {
  ...
  request.task.hostAdapterId = `admitted-portable-launcher-${family}`;
...
async function buildFamilyRecord(family, godskillsRoot) {
  ...
  const admitted = await setupAdmittedIdentity(null, `admitted-portable-launcher-${family}`);
  try {
    const providerPolicy = definition.policy();
    providerPolicy.policyId = `admitted-portable-launcher-${family}-provider-v1`;
    ...
    const provider = await createProviderPhaseHost({
      ...
      clock: increasingIsoClock('2026-09-05T23:40:00.000Z', 100),
      fetchImpl: async (url, providerRequest) => {
        const body = JSON.parse(providerRequest.body);
        const input = definition.input(body);
        const ordinal = calls.length + 1;
        calls.push({
          url: String(url),
          credentialMatches: definition.credentialMatches(providerRequest.headers),
          requestDigest: sha256Text(providerRequest.body),
          phase: input.phase,
        });
    ...
    const portableDescription = buildPortablePhaseHostDescription({
      adapterId: `provider-wrapper-${family}`,
      adapterVersion: '1',
      policyDigest: providerPolicyDigest,
    ...
    const dependencyDescription = description.portableDependencies;
    ...
    const policy = {
      schemaVersion: 1,
      policyId: `admitted-portable-launcher-${family}-identity-v1`,
      runtime: {
        ...
        snapshotPath: normalizedRelative(
          policyRoot,
          join(admitted.admissionRoot, 'vessel', 'snapshot.json'),
        ),
        ...
        nativeTransport: structuredClone(dependencyDescription.dependencies.nativeTransport),
        reviewExecutor: structuredClone(dependencyDescription.dependencies.reviewExecutor),
        revisionExecutor: structuredClone(dependencyDescription.dependencies.revisionExecutor),
        limits: {
          timeoutMs: 30_000,
    ...
    const identityPolicyDigest = sha256Text(canonicalJson(policy));
    ...
    const launchRequest = {
      ...
      clock: increasingClock('2026-09-05T23:00:00.000Z', 600_000),
      godskillsClock: increasingIsoClock('2026-09-05T22:00:00.000Z', 100),
    ...
    const completed = await launcher.launch(launchRequest);
    const beforeReplay = { provider: calls.length, ...launches };
    const reconstructedLauncher = await createAdmittedPortableIdentityLauncher(launcherConfiguration);
    const replay = await reconstructedLauncher.launch(launchRequest);
    const afterReplay = { provider: calls.length, ...launches };

    let credentialLeaks = 0;
    for (const path of await allFiles(admitted.root)) {
      const metadata = await lstat(path);
      if (metadata.isFile() && (await readFile(path)).includes(Buffer.from(SECRET))) {
        credentialLeaks += 1;
      }
    }
    const phases = calls.map(({ phase }) => phase);
    const assertions = {
      providerCallsDuringConstruction: providerCallsAfterConstruction
        - providerCallsBeforeConstruction,
      exactPortableHostDescriptionBound: canonicalJson(dependencyDescription.portableHost)
        === canonicalJson(host.describe()),
      exactPolicyDependenciesBound: canonicalJson({
        ...
        native: dependencyDescription.dependencies.nativeTransport,
        review: dependencyDescription.dependencies.reviewExecutor,
        revision: dependencyDescription.dependencies.revisionExecutor,
      }),
      allCredentialsReachedOnlyHeaders: calls.every(({ credentialMatches }) => credentialMatches),
      exactReviewedPhaseOrder: canonicalJson(phases)
      ...
      launcherReconstructed: canonicalJson(description)
        === canonicalJson(reconstructedLauncher.describe()),
    ...
    for (const [name, value] of Object.entries(assertions)) {
      ...
      requireCondition(value === (expectsZero ? 0 : true), `${name} is ${String(value)}`);
    }
    return {
      family,
      providerPolicyDigest,
      identityPolicyDigest,
      launcherBindingDigest: description.bindingDigest,
      dependencyBindingDigest: dependencyDescription.bindingDigest,
      portableHostDescriptionDigest: dependencyDescription.portableHost.descriptionDigest,
      descriptors: {
        native: dependencyDescription.dependencies.nativeTransport.descriptorDigest,
        review: dependencyDescription.dependencies.reviewExecutor.descriptorDigest,
        revision: dependencyDescription.dependencies.revisionExecutor.descriptorDigest,
      },
      execution: {
        phaseOrder: phases,
...
export async function buildDeterministicAdmittedPortableIdentityLauncherFixture({
  ...
  for (const family of Object.keys(FAMILIES)) {
    try {
      ...
      throw new Error(`admitted portable family ${family} failed`, { cause: error });
    }
  }
  ...
  const assertions = {
    ...
    reviewedPhases: records.reduce(
      (sum, record) => sum + record.execution.phaseOrder.length,
      0,
    ),
    replayProviderCalls: records.reduce(
      (sum, record) => sum + record.assertions.replayProviderCalls,
      0,
    ),
    replayRouteLaunches: records.reduce(
      (sum, record) => sum + record.assertions.replayRouteLaunches,
      0,
    ),
    replayActivationLaunches: records.reduce(
      (sum, record) => sum + record.assertions.replayActivationLaunches,
      0,
    ),
    credentialLeaks: records.reduce(
      (sum, record) => sum + record.assertions.credentialLeaks,
      0,
    ),
    ...
    allLaunchersReconstructed: records.every(
      (record) => record.assertions.launcherReconstructed,
    ),
    allPortableHostDescriptionsBound: records.every(
      (record) => record.assertions.exactPortableHostDescriptionBound,
    ),
  };
  ...
  requireCondition(assertions.allFamiliesCompleted, 'one portable family did not complete');
  requireCondition(assertions.allFamiliesAuthorityClosed, 'one portable family expanded authority');
  requireCondition(assertions.allLaunchersReconstructed, 'one launcher did not reconstruct exactly');
  requireCondition(assertions.allPortableHostDescriptionsBound, 'one portable host was not bound exactly');
