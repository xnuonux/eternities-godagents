...
const FIXTURE_PROTOCOL = 'eternities-admitted-provider-backed-identity-launcher-fixture-v1';
const SECRET = 'admitted-provider-backed-identity-launcher-certification-secret';

function requireCondition(condition, message) {
  if (!condition) throw new Error(`admitted provider-backed launcher fixture failed: ${message}`);
}
...
function phaseOutput(input) {
  if (input.phase === 'native') {
    return { content: 'admitted provider-backed draft with one evidence gap' };
  }
  if (input.phase === 'revision') {
    return {
      addressedFindingIds: ['bind-evidence'],
      content: 'admitted provider-backed revision with the evidence gap repaired exactly',
    };
  }
...
const FAMILIES = Object.freeze({
  'openai-compatible-chat-completions-v1': Object.freeze({
    ...
    response(model, phase, ordinal, content) {
      return {
        id: `chatcmpl-admitted-provider-launcher-${phase}-${ordinal}`,
  ...
  'anthropic-messages-v1': Object.freeze({
    ...
    response(model, phase, ordinal, content) {
      return {
        id: `msg-admitted-provider-launcher-${phase}-${ordinal}`,
        type: 'message',
        role: 'assistant',
        model,
...
function requestFixture(family) {
  ...
  request.task.hostAdapterId = `admitted-provider-launcher-${family}`;
...
async function buildFamilyRecord(family, godskillsRoot) {
  ...
  const admitted = await setupAdmittedIdentity(null, `admitted-provider-launcher-${family}`);
  try {
    const providerPolicy = definition.policy();
    providerPolicy.policyId = `admitted-provider-launcher-${family}-provider-v1`;
    ...
    const launcherConfiguration = {
      ...
      executorIdPrefix: `admitted-provider-launcher-${family}`,
    };
    const providerCallsBeforeConstruction = calls.length;
    const launcher = await createAdmittedProviderBackedIdentityLauncher(launcherConfiguration);
    const description = launcher.describe();
    const providerCallsAfterConstruction = calls.length;
    const dependencyDescription = description.providerBackedDependencies;
    ...
    const policy = {
      schemaVersion: 1,
      policyId: `admitted-provider-launcher-${family}-identity-v1`,
      runtime: {
    ...
    const reconstructedLauncher = await createAdmittedProviderBackedIdentityLauncher(
    ...
    const assertions = {
      ...
      exactHostDescriptionBound: canonicalJson(dependencyDescription.provider)
    ...
    return {
      ...
      providerDescriptionDigest: dependencyDescription.provider.descriptionDigest,
      descriptors: {
...
export async function buildDeterministicAdmittedProviderBackedIdentityLauncherFixture({
  ...
  for (const family of Object.keys(FAMILIES)) {
    try {
      ...
      throw new Error(`admitted provider-backed family ${family} failed`, { cause: error });
    }
  }
