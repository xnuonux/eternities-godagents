...
const FIXTURE_PROTOCOL = 'eternities-openai-compatible-admitted-host-fixture-v1';
const SECRET = 'certification-canary-phase-secret-never-persist';

function requireCondition(condition, message) {
  if (!condition) throw new Error(`OpenAI-compatible admitted host fixture failed: ${message}`);
}
...
function providerEnvelope({ ordinal, phase, content }) {
  return {
    id: `chatcmpl-admitted-${phase}-${ordinal}`,
...
function phaseOutput(input) {
  if (input.phase === 'native') {
    return {
      content: `identity-bound draft by ${input.modelProjection.identity.name} with one evidence gap`,
    };
  }
...
function requestFixture() {
  ...
  request.task.hostAdapterId = 'godagents-openai-compatible-admitted-v1';
...
export async function buildDeterministicOpenAICompatibleAdmittedHostFixture({
  godskillsRoot = 'C:/dev/eternities-godskills',
} = {}) {
  const admitted = await setupAdmittedIdentity(null, 'openai-compatible-admitted-host-certification');
  try {
    const providerPolicy = validOpenAICompatiblePhasePolicy();
    providerPolicy.policyId = 'openai-compatible-admitted-host-certification-v1';
    ...
    const identityPolicy = {
      schemaVersion: 1,
      policyId: 'openai-compatible-admitted-host-certification-v1',
      runtime: {
    ...
    const common = {
      ...
      nativeTransport: phaseTransport.native,
      reviewExecutor,
      revisionExecutor,
    ...
    const completionFiles = files.filter((path) => path.endsWith('completion.json')
      && path.includes(`${join('phase-operations', '')}`)).length;
    const phases = networkCalls.map(({ phase }) => phase);
    const assertions = {
      exactProviderPolicyPinned: env.GODAGENT_PHASE_TRANSPORT_POLICY_SHA256 === providerPolicyDigest,
      identityPolicyPinsNativeDescriptor: canonicalJson(identityPolicy.runtime.nativeTransport)
        === canonicalJson(phaseTransport.descriptors.native),
      allCredentialsReachedOnlyHeaders: networkCalls.every(({ authorizationMatches }) => authorizationMatches),
      exactReviewedPhaseOrder: canonicalJson(phases) === canonicalJson(['native', 'review', 'revision', 'review']),
      fourDurablePhaseCompletions: completionFiles === 4,
    ...
    for (const [name, value] of Object.entries(assertions)) {
      const expectsZero = name.startsWith('replay') || name === 'secretLeaks';
    ...
    const unsigned = {
      ...
      descriptors: {
        native: phaseTransport.descriptors.native.descriptorDigest,
        review: phaseTransport.descriptors.review.descriptorDigest,
        revision: phaseTransport.descriptors.revision.descriptorDigest,
      },
      execution: {
        phaseOrder: phases,
        requestDigests: networkCalls.map(({ requestDigest }) => requestDigest),
