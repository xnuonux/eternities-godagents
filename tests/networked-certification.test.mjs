...
function input(requirements = evidence()) {
  return {
    ...
    proof: {
      ...
      networkIsolation: { status: 'pass', basis: ['src/certification/no-network-guard.mjs'] },
...
test('networked certification requires NC-001 through NC-010 and all proof gates', () => {
  ...
  assert.equal(receipt.proof.networkIsolation.status, 'pass');
...
test('missing requirement or failed proof gate blocks networked certification', () => {
  ...
  for (const gate of ['canaryContainment', 'networkIsolation', 'historicalReceipt']) {
