...
The native verifier inertly checks changed, escaping, noncanonical, observed symbolic-link, and syntactically import-bearing artifacts. Before any module evaluation, the launcher binds the complete descriptor set to an externally pinned policy and verifies that policy against admitted genesis. It then imports the exact bytes already verified through data URLs keyed by bundle, capability, and source digest. The crash fixture crosses a fresh Node process boundary, proves persisted Muse recovery invokes only Forge, and proves terminal replay invokes no executor. Receipt-certified code remains trusted inside Node. Hostile same-user replacement races and hard-link identity remain outside proof. This remains an opt-in programmatic mechanism, not a provider-quality claim, operating-system sandbox, external exactly-once guarantee, or default launch path.
