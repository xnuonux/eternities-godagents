...
Version `0.2.0` preserves the certified local v0 vessel and separately proves a networked-cortex boundary, a modular Phase 1 creation forge, a Phase 2 transactional genesis boundary, a headless Phase 3 creator protocol, the first three host-side phases of the Cortex Binding Protocol, and a provider-neutral identity-bound mission vessel. The binding path reconstructs one inert identity candidate from an admitted genesis, binds governed Godskills once, places the exact bounded identity projection beside the exact recoverable mission package, and closes native generation through review, revision, and final review without granting Realm or continuity authority. Evolution and Soul activation remain excluded.
...
`npm run certify:identity-bound-mission-vessel` reproduces the fixture and issues the next append-only receipt. It proves this local composition against injected transports that promise terminal lookup and atomic deduplication. It does not prove a live model or provider, output quality, those transport guarantees themselves, hostile same-user isolation, credentials, model routing, a concrete Codex or Claude adapter, Realm action, continuity admission, Lunari integration, Inspiration, or Soul. A crash after first external Godskills activation but before vessel-record publication can still repeat an activation whose own adapter must be idempotent by request digest.
