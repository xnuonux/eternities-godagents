...
The native verifier inertly checks changed, escaping, noncanonical, observed symbolic-link, and syntactically import-bearing artifacts. Before any executor evaluation, the launcher binds the complete descriptor set to an externally pinned policy and verifies that policy against admitted genesis. Acorn requires one exported async function as the entire program. Its verified body runs per invocation in a fresh restricted VM context with canonical JSON input and output, no injected Node loader or host object, and dynamic string or WebAssembly code generation disabled. The crash fixture crosses a fresh Node process boundary, proves persisted Muse recovery invokes only Forge, and proves terminal replay invokes no executor. Receipt-certified code remains trusted. Hostile same-user replacement races and hard-link identity remain outside proof. This remains an opt-in programmatic mechanism, not a provider-quality claim, operating-system sandbox, external exactly-once guarantee, or default launch path.
