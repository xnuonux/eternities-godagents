...
The [provider phase host SDK design](docs/superpowers/specs/2026-08-31-provider-phase-host-sdk-v1-design.md) exposes both certified provider families through one explicit common-intersection factory. A caller must name `openai-compatible-chat-completions-v1` or `anthropic-messages-v1`; no policy inspection, environment value, model name, or ambient default chooses the family. Each host provides only a credential-free description, outer credential preflight, and native, review, and revision ports.

The description binds the exact policy digest, all three descriptor digests, family-specific transport identifiers, and honest capability differences. The shared conformance harness runs all three phases and exact completed replay for both families without granting provider-specific extensions through the common surface. OpenAI signed ambiguity resolution remains on its direct module, while Anthropic completion-bound cache evidence remains explicit rather than fabricated as universal parity.
